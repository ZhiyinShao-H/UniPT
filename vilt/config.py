...
def config():
    ...
    max_text_len = 40
    tokenizer = "/root/paddlejob/workspace/env_run/output/shaozhiyin/data/bert_base_uncased"
    vocab_size = 30522
    ...
    data_root = ""
    log_dir = "/root/paddlejob/workspace/env_run/output/shaozhiyin/experiment/data/result"
...
@ex.named_config
def task_mlm_cl_coco():
    exp_name = "mlm_cl"
    datasets = ["coco"]
    loss_names = _loss_names({"cl": 1, "mlm": 1})
    batch_size = 512
    max_epoch = 10
    max_image_len = 200
    max_steps = None
...
@ex.named_config
def task_mlm_cl_LPP_100():
    exp_name = "mlm_cl_LPP_100_1e5"
    datasets = ["LPP"]
    loss_names = _loss_names({"cl": 1, "mlm": 1})
    batch_size = 512
    max_epoch = 10
...
@ex.named_config
def task_mlm_cl_LPP_100_max_1e5():
    exp_name = "mlm_cl_LPP_100_max_1e5"
    datasets = ["LPP"]
    loss_names = _loss_names({"cl": 1, "mlm": 1})
    batch_size = 512
    max_epoch = 10
...
@ex.named_config
def task_mlm_cl_LPP_100_max_1e5_uniimage_3():
    exp_name = "mlm_cl_LPP_100_max_1e5_uniimage_3"
    datasets = ["LPP"]
    loss_names = _loss_names({"cl": 1, "mlm": 1})
    batch_size = 512
    max_epoch = 15
...
@ex.named_config
def task_mlm_cl_LPP_100_max_1e5_2stream_allinit_Resize_size384128_BERT_CAECLIPViT():
    exp_name = "mlm_cl_LPP_100_max_1e5_2stream_allinit_Resize_size384128_BERT_CAECLIPViT"
    datasets = ["LPP"]
    loss_names = _loss_names({"cl": 1, "mlm": 1})
    batch_size = 512
    max_epoch = 15
