...
def compute_cl(pl_module, batch):
    infer = pl_module.infer(batch, mask_text=False, mask_image=False)
    text_feature = infer["text_feature"]
    image_feature = infer["image_feature"]
    mae_loss = infer["mae_loss"]

    text_feature_norm = text_feature/(text_feature.norm(dim=1,keepdim=True) + 1e-12)
    image_feature_norm = image_feature/(image_feature.norm(dim=1,keepdim=True) + 1e-12)

    similarity = torch.mm(image_feature_norm, text_feature_norm.t())
    batch_size = similarity.size(0)
    labels = torch.arange(batch_size, device=similarity.device).long()
    
    cl_loss = (
        F.cross_entropy(similarity,labels) + 
        F.cross_entropy(similarity.t(),labels)
    ) / 2
    cl_loss = cl_loss + mae_loss
    ret = {
        "cl_loss": cl_loss,
        "cl_logits": similarity,
        "cl_labels": labels,
    }
    phase = "train" if pl_module.training else "val"
    loss = getattr(pl_module, f"{phase}_cl_loss")(ret["cl_loss"])
    acc = getattr(pl_module, f"{phase}_cl_accuracy")(
        ret["cl_logits"], ret["cl_labels"]
    )
    pl_module.log(f"cl/{phase}/loss", loss)
    pl_module.log(f"cl/{phase}/accuracy", acc)
    return ret
